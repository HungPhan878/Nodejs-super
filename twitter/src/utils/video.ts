...
export const checkVideoHasAudio = async (filePath: string) => {
  ...
  // I have to specify the path to the file ffmpeg.exe because zx default path ffmpeg at usr/bin/bash
  const ffprobePath = slash(path.resolve('D:/workspace/dowload/ffmpeg/ffprobe.exe'))
  const { stdout } = await $`${ffprobePath} ${[
    '-v',
    'error',
    '-select_streams',
    'a:0',
...
const getBitrate = async (filePath: string) => {
  const { $ } = await import('zx')
  const slash = (await import('slash')).default
  // I have to specify the path to the file ffprobe.exe because zx default path ffmpeg at usr/bin/bash
  const ffprobePath = slash(path.resolve('D:/workspace/dowload/ffmpeg/ffprobe.exe'))
  const { stdout } = await $`${ffprobePath} ${[
    '-v',
    'error',
...
const encodeMax720 = async ({
  ...
  const { $ } = await import('zx')
  const slash = (await import('slash')).default
  const ffmpegPath = slash(path.resolve('D:/workspace/dowload/ffmpeg/ffmpeg.exe'))
